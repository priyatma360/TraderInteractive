...
public class DealerHelper extends DriverHelper {
	...
	public void selectAllRefineFilter() {
		...
		clickOn(makeLink);
		String refineOptionOnPopUp = "(//div[@class='popover-content']//label[contains(@class,'inlineblock')]/span)[1]";
		waitForElementPresentInDom(2);
		clickOn(refineOptionOnPopUp);
		waitForElementPresentInDom(1);
		String update="(//*[@data-track='Refine-Add To Search'])[5]";
		clickOn(update);
		print("Make refine option selected");
		waitForElementPresentInDom(2);
		String modelLink=Locator.AdvancedSearch.Advanced_Model.value;
		clickOn(modelLink);
		waitForElementPresentInDom(2);
		clickOn(refineOptionOnPopUp);
		waitForElementPresentInDom(2);
		clickOn(update);
		print("Model refine option selected");
		waitForElementPresentInDom(2);
		String categoryLink = Locator.AdvancedSearch.Advanced_Category.value;
		clickOn(categoryLink);
		waitForElementPresentInDom(2);
		String listingCount= "(//div[@class='popover-content']//label[contains(@class,'inlineblock')]/span)[2]";
		String getcount= getText(listingCount);
		String result = getcount.replaceAll("[()-+.^:,]","");
		print(result);
		clickOn(refineOptionOnPopUp);
		waitForElementPresentInDom(1);
		clickOn(update);
		waitForElementPresentInDom(2);
		String listingResult="//*[@id='searchPage']//b/span";
		String txt = getText(listingResult).replaceAll("[()-+.^:,]", "");
		print(txt);
